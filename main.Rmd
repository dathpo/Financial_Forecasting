...
```
```{r, fig.width=11, fig.height=4}
ggtsdisplay(train.df$y, main="Autocorrelation and Partial Autocorrelation",
            points=F, xlab="Time", ylab="Return")
ggsave("train_arima.pdf", width=8, height=4)
```
```{r, fig.width=11, fig.height=4}
train.weekly.ts <- ts(train.df$y, frequency = 7)
train.yearly.ts <- ts(train.df$y, frequency = 365.25)
train.yearly.ts %>% decompose %>% autoplot
ggsave("train_decomp.pdf", width=8, height=4)
```
```{r, fig.width=11, fig.height=4}
y <- train.weekly.ts
...
ggtitle("BTC-USD Daily Returns - Training Set: 10-Day Moving Average")
...
ggtitle("BTC-USD Daily Returns - Training Set: Neural Network Actual vs Predicted Values")
...
xlab("Date") + ylab("Return") + scale_y_continuous(labels=percent) + 
scale_colour_manual(name="Legend", values = c("#377EB8", "#F8766D", "#E41A1C")) +
ggtitle("BTC-USD Daily Returns - Test Set: Actual vs Predicted Values")
ggsave("test_pred.pdf", width=8, height=4)
```
```{r, fig.width=11, fig.height=4}
